...
class Provider extends AbstractProvider
{
    ...
    /**
     * Azure does not allow ',' in scopes
     */
    ...
    public function getScopes()
    {
        ...
        if (!env('AADB2C_ClientId')) {
            $dotenvPath = $_SERVER['DOCUMENT_ROOT'] . '/.env';
            if (file_exists($dotenvPath)) {
                $dotenv = \Dotenv\Dotenv::createImmutable(dirname($dotenvPath));
                $dotenv->load();
            } else {
                throw new \Exception('Unable to locate .env file');
            }
        }

        $clientIdScope = env('AADB2C_ClientId');
        return array_merge($this->scopes, [$clientIdScope]);
    }
    ...
    public function user()
    {
        ...
        $_user->setToken(Arr::get($response, 'access_token'));
        return $_user;
    }
